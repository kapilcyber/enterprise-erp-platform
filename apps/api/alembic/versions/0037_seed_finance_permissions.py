"""Seed finance permissions and roles."""
...
from modules.finance.permissions import (
    CFO_PERMISSIONS,
...
revision: str = "0037_seed_finance_permissions"
down_revision: str | None = "0036_fin_asset_transaction"
...
ROLE_SPECS: list[tuple[str, str, list[str]]] = [
    ("FINANCE_ACCOUNTANT", "Finance Accountant", FINANCE_ACCOUNTANT_PERMISSIONS),
    ("FINANCE_MANAGER", "Finance Manager", FINANCE_MANAGER_PERMISSIONS),
    ("CFO", "Chief Financial Officer", CFO_PERMISSIONS),
]


def _ensure_permission(conn, now: datetime, code: str, resource: str, action: str, module: str) -> str:
    exists = conn.execute(
...
def _ensure_role(conn, now: datetime, tenant_id: str, role_code: str, role_name: str) -> str:
    exists = conn.execute(
        sa.text(
            """
...
def upgrade() -> None:
    ...
    for code, resource, action, module in FINANCE_PERMISSIONS:
    ...
    for (tenant_id,) in tenants:
        ...
        super_admin = conn.execute(
            sa.text(
                """
                SELECT id FROM foundation.sec_role
                WHERE tenant_id = :tid AND role_code = 'SUPER_ADMIN' AND is_deleted = false
                LIMIT 1
                """
            ),
            {"tid": tid},
        ...
        if super_admin:
            for perm_id in perm_map.values():
                _grant(conn, now, tid, str(super_admin[0]), perm_id)


def downgrade() -> None:
    conn = op.get_bind()
    role_codes = [r[0] for r in ROLE_SPECS]
    perm_codes = [p[0] for p in FINANCE_PERMISSIONS]
