"""Phase 1.5: BPM polish columns, indexes, permissions."""
...
revision: str = "0471_bpm_phase15_polish"
down_revision: str | None = "0470_seed_bpm_permissions"
...
ROLE_SPECS: list[tuple[str, str, list[str]]] = [
    ("BPM_ADMIN", "BPM Admin", BPM_ADMIN_PERMISSIONS),
    ("PROCESS_DESIGNER", "Process Designer", PROCESS_DESIGNER_PERMISSIONS),
    ("PROCESS_OWNER", "Process Owner", PROCESS_OWNER_PERMISSIONS),
    ("WORKFLOW_OPERATOR", "Workflow Operator", WORKFLOW_OPERATOR_PERMISSIONS),
    ("WORKFLOW_AUDITOR", "Workflow Auditor", WORKFLOW_AUDITOR_PERMISSIONS),
]
...
def upgrade() -> None:
    # Audit reason columns on existing version table (no new ERD table)
    op.add_column(
    ...
    op.add_column(
        "bpm_workflow_version",
        sa.Column("clone_reason", sa.Text(), nullable=True),
        schema="bpm",
    )

    # Search / sort indexes
    op.create_index(
        "ix_bpm_workflow_category_name_search",
        "bpm_workflow_category",
        ["company_id", "category_name"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_category_code_search",
        "bpm_workflow_category",
        ["company_id", "category_code"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_template_name_search",
        "bpm_workflow_template",
        ["company_id", "template_name"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_template_code_search",
        "bpm_workflow_template",
        ["company_id", "template_code"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_template_module",
        "bpm_workflow_template",
        ["company_id", "module_code"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_template_updated",
        "bpm_workflow_template",
        ["company_id", "updated_at"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_definition_name_search",
        "bpm_workflow_definition",
        ["company_id", "definition_name"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_definition_code_search",
        "bpm_workflow_definition",
        ["company_id", "definition_code"],
        schema="bpm",
    )
    op.create_index(
        "ix_bpm_workflow_definition_template",
        "bpm_workflow_definition",
        ["template_id"],
        schema="bpm",
    )

    # Seed Phase 1.5 permissions
    conn = op.get_bind()
    now = datetime.now(timezone.utc)
    for code, resource, action, module in BPM_PERMISSIONS:
        if code in PHASE15_PERMS or True:
...
def downgrade() -> None:
    for name, table in [
        ("ix_bpm_workflow_definition_template", "bpm_workflow_definition"),
        ("ix_bpm_workflow_definition_code_search", "bpm_workflow_definition"),
        ("ix_bpm_workflow_definition_name_search", "bpm_workflow_definition"),
        ("ix_bpm_workflow_template_updated", "bpm_workflow_template"),
        ("ix_bpm_workflow_template_module", "bpm_workflow_template"),
        ("ix_bpm_workflow_template_code_search", "bpm_workflow_template"),
        ("ix_bpm_workflow_template_name_search", "bpm_workflow_template"),
        ("ix_bpm_workflow_category_code_search", "bpm_workflow_category"),
        ("ix_bpm_workflow_category_name_search", "bpm_workflow_category"),
    ]:
        op.drop_index(name, table_name=table, schema="bpm")

    op.drop_column("bpm_workflow_version", "clone_reason", schema="bpm")
    op.drop_column("bpm_workflow_version", "retire_reason", schema="bpm")
    op.drop_column("bpm_workflow_version", "publish_reason", schema="bpm")

    conn = op.get_bind()
    for code in PHASE15_PERMS:
        conn.execute(
