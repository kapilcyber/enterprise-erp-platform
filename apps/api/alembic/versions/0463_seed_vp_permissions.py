"""Seed Vendor Portal permissions and roles per ERD_24."""
...
revision: str = "0463_seed_vp_permissions"
down_revision: str | None = "0462_vp_report"
...
ROLE_SPECS: list[tuple[str, str, list[str]]] = [
    ("VENDOR_PORTAL_ADMIN", "Vendor Portal Admin", VENDOR_PORTAL_ADMIN_PERMISSIONS),
    ("PROCUREMENT_MANAGER", "Procurement Manager", PROCUREMENT_MANAGER_PERMISSIONS),
    ("SUPPLIER_USER", "Supplier User", SUPPLIER_USER_PERMISSIONS),
    ("QUALITY_COORDINATOR", "Quality Coordinator", QUALITY_COORDINATOR_PERMISSIONS),
]
...
def upgrade() -> None:
    ...
    for code, resource, action, module in VENDOR_PORTAL_PERMISSIONS:
...
def downgrade() -> None:
    ...
    for code, *_ in VENDOR_PORTAL_PERMISSIONS:
        conn.execute(
