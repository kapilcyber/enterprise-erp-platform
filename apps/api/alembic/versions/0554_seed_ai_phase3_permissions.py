"""Seed AI Platform Phase 3 agent/tool permissions and designer role."""
...
revision: str = "0554_seed_ai_phase3_permissions"
down_revision: str | None = "0553_ai_agent_version"
...
ROLE_SPECS: list[tuple[str, str, list[str]]] = [
    ("AI_AGENT_DESIGNER", "AI Agent Designer", AI_AGENT_DESIGNER_PERMISSIONS),
]
...
def upgrade() -> None:
    ...
    for code, resource, action, module in AI_PHASE3_PERMISSIONS:
    ...
    for (tenant_id,) in tenants:
        ...
        for role_code, perms in ROLE_GRANT_SPECS:
            ...
            for pcode in perms:
                if not any(pcode == p[0] for p in AI_PHASE3_PERMISSIONS):
                    continue
                row = conn.execute(
                    sa.text("SELECT id FROM foundation.sec_permission WHERE permission_code = :c"),
                    {"c": pcode},
...
def downgrade() -> None:
    ...
    for code, *_ in AI_PHASE3_PERMISSIONS:
        conn.execute(
