...
revision: str = "0011_seed_org_permissions"
down_revision: str | None = "0010_foundation_org_fks"
...
def upgrade() -> None:
    ...
    for code, resource, action, module in ORGANIZATION_PERMISSIONS:
        exists = conn.execute(
...
def downgrade() -> None:
    conn = op.get_bind()
    codes = [p[0] for p in ORGANIZATION_PERMISSIONS]
    for code in codes:
        conn.execute(
            sa.text(
                """
