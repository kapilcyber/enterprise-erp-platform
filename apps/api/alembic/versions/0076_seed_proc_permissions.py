"""Seed procurement permissions and roles."""
...
revision: str = "0076_seed_proc_permissions"
down_revision: str | None = "0075_proc_vendor_performance"
...
ROLE_SPECS: list[tuple[str, str, list[str]]] = [
    ("BUYER", "Buyer", BUYER_PERMISSIONS),
    ("PROCUREMENT_MANAGER", "Procurement Manager", PROCUREMENT_MANAGER_PERMISSIONS),
    ("FINANCE_REVIEWER_PROC", "Finance Reviewer (Procurement)", FINANCE_REVIEWER_PERMISSIONS),
]
...
def upgrade() -> None:
    ...
    for code, resource, action, module in PROC_PERMISSIONS:
...
def downgrade() -> None:
    ...
    perm_codes = [p[0] for p in PROC_PERMISSIONS]
