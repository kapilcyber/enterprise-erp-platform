"""Seed AI Platform Phase 2 knowledge permissions and curator role."""
...
from modules.ai.permissions import (  # noqa: E402
...
revision: str = "0548_seed_ai_phase2_permissions"
down_revision: str | None = "0547_ai_vector_index"
...
ROLE_SPECS: list[tuple[str, str, list[str]]] = [
    ("AI_KNOWLEDGE_CURATOR", "AI Knowledge Curator", AI_KNOWLEDGE_CURATOR_PERMISSIONS),
]

# Grant phase2 perms to existing roles that should inherit them
ROLE_GRANT_SPECS: list[tuple[str, list[str]]] = [
    ("AI_PLATFORM_ADMIN", AI_PLATFORM_ADMIN_PERMISSIONS),
    ("AI_PUBLISHER", AI_PUBLISHER_PERMISSIONS),
]
...
def upgrade() -> None:
    ...
    for code, resource, action, module in AI_PHASE2_PERMISSIONS:
    ...
    for (tenant_id,) in tenants:
        ...
        for role_code, perms in ROLE_GRANT_SPECS:
            ...
            for pcode in perms:
                if not any(pcode == p[0] for p in AI_PHASE2_PERMISSIONS):
                    continue
                row = conn.execute(
                    sa.text("SELECT id FROM foundation.sec_permission WHERE permission_code = :c"),
                    {"c": pcode},
...
def downgrade() -> None:
    ...
    for role_code, _, _ in ROLE_SPECS:
        conn.execute(
            sa.text("UPDATE foundation.sec_role SET is_deleted = true WHERE role_code = :rc"),
            {"rc": role_code},
        )
    for code, *_ in AI_PHASE2_PERMISSIONS:
        conn.execute(
