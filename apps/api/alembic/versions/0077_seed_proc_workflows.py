"""Seed procurement workflow definitions per ERD_06 §12."""
...
revision: str = "0077_seed_proc_workflows"
down_revision: str | None = "0076_seed_proc_permissions"
...
WORKFLOWS: list[tuple[str, str, str, list[tuple[int, str, str, str]]]] = [
    (
        ...
        "proc_requisition_header",
        [
            (1, "EMPLOYEE_MANAGER", "Employee Manager Review", "role"),
        ],
    ),
    (
        "PROC_RFQ_APPROVAL",
        ...
        "proc_rfq_header",
        [
            (1, "BUYER", "Procurement Executive Review", "role"),
            (2, "PROCUREMENT_MANAGER", "Procurement Manager Approval", "role"),
        ],
    ),
    (
        "PROC_PO_APPROVAL",
        ...
        "proc_order_header",
        [
            (1, "BUYER", "Buyer Review", "role"),
            (2, "PROCUREMENT_MANAGER", "Procurement Manager Approval", "role"),
            (3, "FINANCE_MANAGER", "Finance Manager Approval", "role"),
        ],
    ),
    (
        "PROC_PO_HIGH_VALUE",
        ...
        [
            ...
            (2, "PROCUREMENT_MANAGER", "Procurement Manager Approval", "role"),
            (3, "FINANCE_MANAGER", "Finance Manager Approval", "role"),
            (4, "CFO", "CFO Final Approval", "role"),
        ],
    ),
    (
        "PROC_INVOICE_APPROVAL",
        "Purchase Invoice Approval",
        "proc_invoice_header",
        [
            (1, "FINANCE_REVIEWER_PROC", "Finance Review", "role"),
        ],
    ),
    (
        "PROC_RETURN_APPROVAL",
        "Purchase Return Approval",
        "proc_return_header",
        [
            (1, "PROCUREMENT_MANAGER", "Procurement Manager Review", "role"),
            (2, "FINANCE_MANAGER", "Finance Manager Approval", "role"),
        ],
    ),
    (
        "PROC_CONTRACT_APPROVAL",
        "Vendor Contract Approval",
        "proc_vendor_contract",
        [
            (1, "PROCUREMENT_MANAGER", "Procurement Review", "role"),
            (2, "FINANCE_MANAGER", "Finance Approval", "role"),
        ],
    ),
]
            ...
            if exists:
                ...
                conn.execute(
                    sa.text(
                        ...
                        VALUES (:id, :tid, :code, :name, 'procurement', :doc, 1, true, :now, :now)
                        """
                    ),
                    {
                        "id": wf_id,
