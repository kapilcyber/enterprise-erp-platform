...
from modules.integration.repository.sync_job_repository import SyncJobRepository
from modules.integration.service.engines import SyncJobEngine
...
class SyncJobService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> IntSyncJob:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("SyncJobService not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        doc = self._numbers.generate(IntegrationEntityType.SYNC_JOB, cid, IntSyncJob, "sync_number")
        return self._repo.create(ctx, company_id=cid, sync_number=doc, **fields)
