"""HR HrShiftAssignment repository."""
...
class ShiftAssignmentRepository(HrScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    def get(self, ctx: TenantContext, row_id: UUID) -> HrShiftAssignment | None:
        stmt = select(HrShiftAssignment).where(HrShiftAssignment.id == row_id, HrShiftAssignment.is_deleted.is_(False))
    ...
    def update(self, ctx: TenantContext, row_id: UUID, **fields) -> HrShiftAssignment | None:
