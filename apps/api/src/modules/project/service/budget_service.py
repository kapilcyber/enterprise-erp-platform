...
from modules.project.repository.project_budget_repository import ProjectBudgetRepository
from modules.project.service.document_number_service import DocumentNumberService
from modules.project.service.engines import ProjectBudgetEngine
from modules.project.service.project_scope_validator import ProjectScopeValidator


class BudgetService:
    def __init__(self, db: Session) -> None:
        self._repo = ProjectBudgetRepository(db)
        self._scope = ProjectScopeValidator(db)
        self._numbers = DocumentNumberService(db)
        self._engine = ProjectBudgetEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> PrjProjectBudget:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("BudgetService not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):

        cid = self._scope.resolve_company_id(ctx, company_id)
        doc = self._numbers.generate(PrjEntityType.PROJECT_BUDGET, cid, PrjProjectBudget, "document_number")
