"""TaskDependencyService application service."""
...
class TaskDependencyService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> PrjTaskDependency:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("TaskDependencyService not found")
        return row
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        ...
        self._audit.log_entity_change(
            ...
            entity_name="prj_task_dependency",
            entity_id=row.id,
            operation="create",
