...
from modules.project.service.engines import ChangeRequestEngine
from modules.project.service.project_scope_validator import ProjectScopeValidator


class ChangeRequestService:
    def __init__(self, db: Session) -> None:
        self._repo = ChangeRequestRepository(db)
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> PrjChangeRequest:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("ChangeRequestService not found")
        return row
    ...
    def create(self, ctx: TenantContext, *, branch_id: UUID, company_id: UUID | None = None, **fields):
        ...
        doc = self._numbers.generate(PrjEntityType.CHANGE_REQUEST, cid, PrjChangeRequest, "document_number")
