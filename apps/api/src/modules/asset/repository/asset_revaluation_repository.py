"""Asset AstAssetRevaluation repository."""
...
class AssetRevaluationRepository(AstScopedRepository):
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> AstAssetRevaluation | None:
        stmt = select(AstAssetRevaluation).where(AstAssetRevaluation.id == row_id, AstAssetRevaluation.is_deleted.is_(False))
    ...
    def list_rows(self, ctx: TenantContext, company_id: UUID):
        stmt = select(AstAssetRevaluation).where(
            AstAssetRevaluation.company_id == company_id,
