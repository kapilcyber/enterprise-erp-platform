"""Inventory repository base utilities."""
...
class InvScopedRepository(OrgScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    @staticmethod
    def apply_inv_filter(stmt, model, ctx: TenantContext, *, branch_scoped: bool = False):
        stmt = InvScopedRepository.apply_tenant_filter(stmt, model, ctx)
    ...
    def resolve_company_id(ctx: TenantContext, company_id: UUID | None) -> UUID:
        if company_id is not None:
            InvScopedRepository.ensure_company_access(ctx, company_id)
            return company_id
