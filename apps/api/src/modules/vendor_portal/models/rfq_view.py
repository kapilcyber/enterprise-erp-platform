"""Vendor Portal ORM — vp_rfq_view per ERD_24."""
...
class VpRfqView(Base, *VpRowMixin):
    __tablename__ = "vp_rfq_view"
    __table_args__ = (
        UniqueConstraint("company_id", "view_number", name="uk_vp_rfq_view_view_number"),
        CheckConstraint(
            "status IN ('visible','hidden','stale','closed')",
            name="ck_vp_rfq_view_status",
        ),
        Index("ix_vp_rfq_view_tenant_co_status", "tenant_id", "company_id", "status"),
    ...
    proc_rfq_vendor_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), nullable=True)
    rfq_ref: Mapped[str | None] = mapped_column(String(100), nullable=True)
    rfq_status_text: Mapped[str | None] = mapped_column(String(50), nullable=True)
