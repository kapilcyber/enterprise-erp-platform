...
from modules.vendor_portal.repository.quote_submission_repository import QuoteSubmissionRepository
from modules.vendor_portal.service.engines import QuoteSubmissionEngine
...
class QuoteSubmissionService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> VpQuoteSubmission:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("QuoteSubmissionService not found")
        return row
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        ...
        doc = self._numbers.generate(
            VendorPortalEntityType.QUOTE_SUBMISSION,
            cid,
            VpQuoteSubmission,
            "submission_number",
        )
