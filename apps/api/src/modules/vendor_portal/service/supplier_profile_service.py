...
from modules.vendor_portal.repository.supplier_profile_repository import SupplierProfileRepository
from modules.vendor_portal.service.engines import SupplierProfileEngine
...
class SupplierProfileService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> VpSupplierProfile:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("SupplierProfileService not found")
        return row
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        ...
        doc = self._numbers.generate(
            VendorPortalEntityType.SUPPLIER_PROFILE,
            cid,
            VpSupplierProfile,
            "profile_number",
        )
