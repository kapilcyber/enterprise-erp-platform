"""DeveloperTeamService — Phase 1 CRUD."""
...
class DeveloperTeamService:
    def __init__(self, db: Session) -> None:
        self._repo = DeveloperTeamRepository(db)
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> DpDeveloperTeam:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("DeveloperTeam not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)

        code = fields.pop("team_code", None) or f"TEAM-{uuid4().hex[:8].upper()}"
        fields.setdefault("team_code", code)
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived DeveloperTeam not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="dp_developer_team",
            entity_id=row.id,
