"""DeveloperAccountService — Phase 1 CRUD + lifecycle."""
...
from modules.devportal.service.engines import AccountLifecycleEngine
...
class DeveloperAccountService:
    def __init__(self, db: Session) -> None:
        self._repo = DeveloperAccountRepository(db)
        self._scope = DevportalScopeValidator(db)
        self._audit = AuditService(db)
        self._engine = AccountLifecycleEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> DpDeveloperAccount:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("DeveloperAccount not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)

        code = fields.pop("account_code", None) or f"ACC-{uuid4().hex[:8].upper()}"
        fields.setdefault("account_code", code)
        ...
        self._audit.log_entity_change(
            ...
            entity_name="dp_developer_account",
            entity_id=row.id,
            operation="create",
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived DeveloperAccount not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="dp_developer_account",
            entity_id=row.id,
    ...
    def lock(self, ctx: TenantContext, row_id: UUID, **kwargs):
        row = self.get(ctx, row_id)
