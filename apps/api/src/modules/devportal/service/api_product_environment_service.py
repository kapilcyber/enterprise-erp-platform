"""ApiProductEnvironmentService — Phase 1 CRUD."""
...
class ApiProductEnvironmentService:
    def __init__(self, db: Session) -> None:
        self._repo = ApiProductEnvironmentRepository(db)
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> DpApiProductEnvironment:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("ApiProductEnvironment not found")
        return row
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        ...
        self._audit.log_entity_change(
            ...
            entity_name="dp_api_product_environment",
            entity_id=row.id,
            operation="create",
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived ApiProductEnvironment not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="dp_api_product_environment",
            entity_id=row.id,
