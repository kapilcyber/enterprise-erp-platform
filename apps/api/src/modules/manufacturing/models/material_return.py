"""Manufacturing material return ORM models."""
...
class MfgMaterialReturn(Base, *MfgTransactionMixin):
    __tablename__ = "mfg_material_return"
    __table_args__ = (
        UniqueConstraint("company_id", "document_number", name="uk_mfg_mr_company_number"),
        CheckConstraint(
            "status IN ('draft','confirmed','cancelled')",
            name="ck_mfg_mr_status",
        ),
    ...
    returned_by: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), nullable=True)
    ...
    lines: Mapped[list["MfgMaterialReturnLine"]] = relationship(
        back_populates="material_return", cascade="all, delete-orphan"
    )


class MfgMaterialReturnLine(Base, *MfgTxnLineMixin):
    __tablename__ = "mfg_material_return_line"
    __table_args__ = (
        UniqueConstraint("material_return_id", "line_number", name="uk_mfg_mr_line"),
        CheckConstraint("quantity > 0", name="ck_mfg_mr_line_qty"),
        {"schema": "manufacturing"},
    )

    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid4)
    material_return_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("manufacturing.mfg_material_return.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
    ...
    material_return: Mapped[MfgMaterialReturn] = relationship(back_populates="lines")
