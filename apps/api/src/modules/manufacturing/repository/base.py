"""Manufacturing repository base utilities."""
...
class MfgScopedRepository(OrgScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    @staticmethod
    def apply_mfg_filter(stmt, model, ctx: TenantContext, *, branch_scoped: bool = False):
        stmt = MfgScopedRepository.apply_tenant_filter(stmt, model, ctx)
    ...
    def resolve_company_id(ctx: TenantContext, company_id: UUID | None) -> UUID:
        if company_id is not None:
            MfgScopedRepository.ensure_company_access(ctx, company_id)
            return company_id
