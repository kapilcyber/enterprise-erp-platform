"""mon_service_policy_assignment ORM per ERD-29 Phase 1."""
...
class MonServicePolicyAssignment(Base, *MonitoringRowMixin):
    __tablename__ = "mon_service_policy_assignment"
    __table_args__ = (
        UniqueConstraint(
            "service_id",
            "component_id",
            "policy_version_id",
            name="uk_mon_service_policy_assignment",
        ),
        CheckConstraint(
            f"status IN ({_STATUSES})", name="ck_mon_service_policy_assignment_status"
        ),
        Index(
            "ix_mon_svc_policy_assign_tenant_co_status",
            "tenant_id",
            "company_id",
            "status",
        ),
    ...
    policy_version_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("monitoring.mon_observability_policy_version.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
