"""ServicePolicyAssignmentService — Phase 1 CRUD + lifecycle."""
...
from modules.monitoring.service.engines import AssignmentLifecycleEngine
...
class ServicePolicyAssignmentService:
    def __init__(self, db: Session) -> None:
        self._repo = ServicePolicyAssignmentRepository(db)
        self._scope = MonitoringScopeValidator(db)
        self._audit = AuditService(db)
        self._engine = AssignmentLifecycleEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("ServicePolicyAssignment not found")
        return row
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        ...
        if service_id is None or policy_version_id is None:
            raise ConflictException(
                "ServicePolicyAssignment requires service_id and policy_version_id"
            )
        ...
        self._audit.log_entity_change(
            ...
            entity_name="mon_service_policy_assignment",
            entity_id=row.id,
            operation="create",
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived ServicePolicyAssignment not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="mon_service_policy_assignment",
            entity_id=row.id,
