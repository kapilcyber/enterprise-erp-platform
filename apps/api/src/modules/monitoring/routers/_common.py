"""Shared Monitoring router utilities — thin handler helpers."""
...
from modules.monitoring.dependencies import (
    PaginationParams,
    SortParams,
...
def _app(db: Session) -> MonitoringApplicationService:
    return MonitoringApplicationService(db)
...
def register_standard_crud(
    ...
    *,
    ...
    read_perm = f"monitoring.{resource}:read"
    create_perm = f"monitoring.{resource}:create"
    update_perm = f"monitoring.{resource}:update"
    archive_perm = f"monitoring.{resource}:archive"
    restore_perm = f"monitoring.{resource}:restore"
...
def register_lifecycle_route(
    ...
    *,
    ...
    perm = f"monitoring.{resource}:{action}"
