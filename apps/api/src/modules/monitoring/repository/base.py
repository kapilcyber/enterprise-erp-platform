"""Monitoring scoped repository base — Phase 0 pagination / sort helpers."""
...
class MonitoringScopedRepository(OrgScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    @staticmethod
    def apply_monitoring_filter(stmt, model, ctx: TenantContext, *, branch_scoped: bool = False):
        stmt = MonitoringScopedRepository.apply_tenant_filter(stmt, model, ctx)
    ...
    def resolve_company_id(ctx: TenantContext, company_id: UUID | None) -> UUID:
        if company_id is not None:
            MonitoringScopedRepository.ensure_company_access(ctx, company_id)
            return company_id
