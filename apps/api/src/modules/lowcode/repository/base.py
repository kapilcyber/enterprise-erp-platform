"""Low-Code scoped repository base — Phase 1 pagination / sort helpers."""
...
class LowcodeScopedRepository(OrgScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    @staticmethod
    def apply_lowcode_filter(stmt, model, ctx: TenantContext, *, branch_scoped: bool = False):
        stmt = LowcodeScopedRepository.apply_tenant_filter(stmt, model, ctx)
    ...
    def resolve_company_id(ctx: TenantContext, company_id: UUID | None) -> UUID:
        if company_id is not None:
            LowcodeScopedRepository.ensure_company_access(ctx, company_id)
            return company_id
