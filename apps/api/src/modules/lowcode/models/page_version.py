"""lc_page_version ORM per ERD-26 Phase 3B — Draft / Published / Retired."""
...
class LcPageVersion(Base, *LowcodeRowMixin):
    __tablename__ = "lc_page_version"
    __table_args__ = (
        UniqueConstraint(
            "definition_id", "version_number", name="uk_lc_page_version_number"
        ),
        CheckConstraint(
            "status IN ('draft','published','retired')",
            name="ck_lc_page_version_status",
        ),
        Index("ix_lc_page_version_definition_status", "definition_id", "status"),
        Index("ix_lc_page_version_tenant_co", "tenant_id", "company_id"),
        {"schema": "lowcode"},
    )
    ...
    # Optional layout shell metadata (JSON text) — no rendering engine
    ...
    cloned_from_version_id: Mapped[UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("lowcode.lc_page_version.id", ondelete="SET NULL"),
        nullable=True,
    )
