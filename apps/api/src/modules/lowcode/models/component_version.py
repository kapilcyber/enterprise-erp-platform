"""lc_component_version ORM per ERD-26 Phase 2B.

Component properties are merged onto this table (properties_json).
"""
...
class LcComponentVersion(Base, *LowcodeRowMixin):
    __tablename__ = "lc_component_version"
    __table_args__ = (
        UniqueConstraint(
            "component_id", "version_number", name="uk_lc_component_version_number"
        ),
        CheckConstraint(
            "status IN ('draft','published','retired')",
            name="ck_lc_component_version_status",
        ),
        Index("ix_lc_component_version_component_status", "component_id", "status"),
        Index("ix_lc_component_version_tenant_co", "tenant_id", "company_id"),
        {"schema": "lowcode"},
    )
    ...
    # Merged typed properties / defaults / validation hooks (JSON text)
    properties_json: Mapped[str | None] = mapped_column(Text, nullable=True)
    default_props_json: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    cloned_from_version_id: Mapped[UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("lowcode.lc_component_version.id", ondelete="SET NULL"),
        nullable=True,
    )
