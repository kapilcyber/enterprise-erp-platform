"""PageDefinitionService — Phase 3B stable identity + initial draft version."""

from uuid import UUID

from sqlalchemy.orm import Session

from core.exceptions import NotFoundException
from modules.foundation.domain.value_objects import TenantContext
from modules.foundation.service.audit_service import AuditService
from modules.lowcode.domain.enums import LowcodeEntityType, VersionStatus
from modules.lowcode.domain.value_objects import PageResult
from modules.lowcode.models import LcPageDefinition, LcPageVersion
from modules.lowcode.repository.form_category_repository import FormCategoryRepository
from modules.lowcode.repository.page_definition_repository import PageDefinitionRepository
from modules.lowcode.repository.page_version_repository import PageVersionRepository
from modules.lowcode.service.engines import PageDefinitionEngine
from modules.lowcode.service.lowcode_number_service import LowcodeNumberService
from modules.lowcode.service.lowcode_scope_validator import LowcodeScopeValidator


class PageDefinitionService:
    def __init__(self, db: Session) -> None:
        self._repo = PageDefinitionRepository(db)
        self._versions = PageVersionRepository(db)
        self._categories = FormCategoryRepository(db)
        self._scope = LowcodeScopeValidator(db)
        self._numbers = LowcodeNumberService(db)
        self._engine = PageDefinitionEngine()
        self._audit = AuditService(db)

    def _assert_category(self, ctx: TenantContext, category_id: UUID | None) -> None:
        if category_id is None:
            return
        if self._categories.get(ctx, category_id) is None:
            raise NotFoundException("Form category not found")

    def list(
        self,
        ctx: TenantContext,
        company_id: UUID | None = None,
        *,
        status: str | None = None,
        category_id: UUID | None = None,
        search: str | None = None,
        page: int = 1,
        page_size: int = 25,
        sort_by: str | None = "page_name",
        sort_dir: str = "asc",
    ) -> PageResult:
        cid = self._scope.resolve_company_id(ctx, company_id)
        return self._repo.list_rows(
            ctx,
            cid,
            status=status,
            category_id=category_id,
            search=search,
            page=page,
            page_size=page_size,
            sort_by=sort_by,
            sort_dir=sort_dir,
        )

    def get(self, ctx: TenantContext, row_id: UUID) -> LcPageDefinition:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("Page definition not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        self._assert_category(ctx, fields.get("category_id"))
        code = fields.pop("page_code", None) or self._numbers.generate(
            LowcodeEntityType.PAGE_DEFINITION, cid, LcPageDefinition, "page_code"
        )
        row = self._repo.create(ctx, company_id=cid, page_code=code, **fields)
        ver_code = self._numbers.generate(
            LowcodeEntityType.PAGE_VERSION, cid, LcPageVersion, "version_code"
        )
        self._versions.create(
            ctx,
            company_id=cid,
            definition_id=row.id,
            version_code=ver_code,
            version_number=1,
            version_label="v1",
            status=VersionStatus.DRAFT.value,
            change_notes="Initial draft",
        )
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="lc_page_definition",
            entity_id=row.id,
            operation="create",
            performed_by=ctx.user_id,
        )
        return row

    def update(self, ctx: TenantContext, row_id: UUID, **fields):
        self.get(ctx, row_id)
        if "category_id" in fields:
            self._assert_category(ctx, fields.get("category_id"))
        row = self._repo.update(ctx, row_id, **fields)
        if row is None:
            raise NotFoundException("Page definition not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="lc_page_definition",
            entity_id=row.id,
            operation="update",
            performed_by=ctx.user_id,
        )
        return row

    def archive(self, ctx: TenantContext, row_id: UUID):
        row = self._repo.soft_delete(ctx, row_id)
        if row is None:
            raise NotFoundException("Page definition not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="lc_page_definition",
            entity_id=row.id,
            operation="archive",
            performed_by=ctx.user_id,
        )
        return row

    def restore(self, ctx: TenantContext, row_id: UUID):
        row = self._repo.restore(ctx, row_id)
        if row is None:
            raise NotFoundException("Archived page definition not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="lc_page_definition",
            entity_id=row.id,
            operation="restore",
            performed_by=ctx.user_id,
        )
        return row

    def activate(self, ctx: TenantContext, row_id: UUID):
        row = self.get(ctx, row_id)
        self._engine.activate(row)
        return self._repo.update(ctx, row_id, status=row.status)

    def retire(self, ctx: TenantContext, row_id: UUID):
        row = self.get(ctx, row_id)
        self._engine.retire(row)
        return self._repo.update(ctx, row_id, status=row.status)
