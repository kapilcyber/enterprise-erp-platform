...
class PayslipRepository(PayScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    def get(self, ctx: TenantContext, row_id: UUID) -> PayPayslip | None:
        stmt = select(PayPayslip).where(PayPayslip.id == row_id, PayPayslip.is_deleted.is_(False))
    ...
    def update(self, ctx: TenantContext, row_id: UUID, **fields) -> PayPayslip | None:
