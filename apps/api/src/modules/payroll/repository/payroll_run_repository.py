"""Payroll PayPayrollRun repository."""
...
class PayrollRunRepository(PayScopedRepository):
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> PayPayrollRun | None:
        stmt = select(PayPayrollRun).where(PayPayrollRun.id == row_id, PayPayrollRun.is_deleted.is_(False))
    ...
    def create(self, ctx: TenantContext, **fields) -> PayPayrollRun:
        row = PayPayrollRun(
            id=uuid4(),
    ...
    def update(self, ctx: TenantContext, row_id: UUID, **fields) -> PayPayrollRun | None:
