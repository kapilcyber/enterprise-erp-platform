"""EmployeeSalaryComponent application service."""
...
from modules.payroll.service.engines import EmployeeSalaryComponentEngine
...
class EmployeeSalaryComponentService:
    def __init__(self, db: Session) -> None:
        self._repo = EmployeeSalaryComponentRepository(db)
        self._scope = PayrollScopeValidator(db)
        self._engine = EmployeeSalaryComponentEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> PayEmployeeSalaryComponent:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("EmployeeSalaryComponent not found")
        return row
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, *, branch_id: UUID | None = None, **fields):
        ...
        self._audit.log_entity_change(
            ...
            entity_name="pay_employee_salary_component",
            entity_id=row.id,
            operation="create",
