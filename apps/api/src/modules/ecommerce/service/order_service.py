...
from modules.ecommerce.repository.order_repository import OrderRepository
...
from modules.ecommerce.service.engines import OrderEngine
...
class OrderService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> EcOrder:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("OrderService not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        doc = self._numbers.generate(EcommerceEntityType.ORDER, cid, EcOrder, "order_number")
        return self._repo.create(ctx, company_id=cid, order_number=doc, **fields)
