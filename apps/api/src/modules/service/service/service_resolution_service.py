...
from modules.service.repository.service_resolution_repository import ServiceResolutionRepository
from modules.service.service.document_number_service import DocumentNumberService
from modules.service.service.engines import ServiceResolutionEngine
from modules.service.service.service_scope_validator import ServiceScopeValidator


class ServiceResolutionService:
    def __init__(self, db: Session) -> None:
        self._repo = ServiceResolutionRepository(db)
        self._scope = ServiceScopeValidator(db)
        self._numbers = DocumentNumberService(db)
        self._engine = ServiceResolutionEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> SvcServiceResolution:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("ServiceResolutionService not found")
        return row
    ...
    def create(self, ctx: TenantContext, *, branch_id: UUID, company_id: UUID | None = None, **fields):
        ...
        doc = self._numbers.generate(SvcEntityType.RESOLUTION, cid, SvcServiceResolution, "document_number")
