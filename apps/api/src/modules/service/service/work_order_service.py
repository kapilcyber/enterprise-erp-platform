...
from modules.service.repository.service_work_order_repository import ServiceWorkOrderRepository
from modules.service.service.document_number_service import DocumentNumberService
from modules.service.service.engines import ServiceWorkOrderEngine
from modules.service.service.service_scope_validator import ServiceScopeValidator


class WorkOrderService:
    def __init__(self, db: Session) -> None:
        self._repo = ServiceWorkOrderRepository(db)
        self._scope = ServiceScopeValidator(db)
        self._numbers = DocumentNumberService(db)
        self._engine = ServiceWorkOrderEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> SvcServiceWorkOrder:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("WorkOrderService not found")
        return row
    ...
    def create(self, ctx: TenantContext, *, branch_id: UUID, company_id: UUID | None = None, **fields):
        ...
        doc = self._numbers.generate(SvcEntityType.WORK_ORDER, cid, SvcServiceWorkOrder, "document_number")
