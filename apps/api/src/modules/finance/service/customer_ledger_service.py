...
class CustomerLedgerService:
    ...
    def list_entries(self, ctx: TenantContext, company_id: UUID | None = None, customer_id: UUID | None = None):
        cid = self._scope.resolve_company_id(ctx, company_id)
        return self._repo.list_customer_ledger(ctx, cid, customer_id)
    ...
    def record_payment(self, ctx: TenantContext, entry_id: UUID, payment_amount: float):
        ...
        return self._repo.update_customer_entry(
            ctx,
            entry_id,
            credit_amount=float(entry.credit_amount) + payment_amount,
