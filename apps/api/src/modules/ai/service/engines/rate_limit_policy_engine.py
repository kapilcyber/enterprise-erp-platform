"""Rate limit policy lifecycle engine — draft / publish / retire / immutability."""
...
from modules.ai.domain.exceptions import (
    ...
    PublishedRateLimitPolicyImmutable,
)
...
class RateLimitPolicyEngine:
    def assert_editable(self, row) -> None:
        if row.status == PolicyStatus.PUBLISHED.value:
            raise PublishedRateLimitPolicyImmutable()
        if row.status == PolicyStatus.RETIRED.value:
            raise InvalidRateLimitPolicyState("Retired rate limit policies are read-only")
        if row.status != PolicyStatus.DRAFT.value:
            raise InvalidRateLimitPolicyState("Only draft rate limit policies are editable")

    def publish(self, row, *, user_id) -> None:
        if row.status != PolicyStatus.DRAFT.value:
            raise InvalidRateLimitPolicyState("Only draft rate limit policies can be published")
    ...
    def retire(self, row, *, user_id) -> None:
        if row.status not in {PolicyStatus.PUBLISHED.value, PolicyStatus.DRAFT.value}:
            raise InvalidRateLimitPolicyState(
                "Only draft or published rate limit policies can be retired"
            )
