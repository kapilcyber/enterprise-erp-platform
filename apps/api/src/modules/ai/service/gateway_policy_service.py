"""GatewayPolicyService — Phase 1 CRUD + publish / retire."""
...
from modules.ai.service.engines import GatewayPolicyEngine
...
class GatewayPolicyService:
    def __init__(self, db: Session) -> None:
        self._repo = GatewayPolicyRepository(db)
        self._scope = AiScopeValidator(db)
        self._numbers = AiNumberService(db)
        self._engine = GatewayPolicyEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> AiGatewayPolicy:
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        ...
        code = fields.pop("policy_code", None) or self._numbers.generate(
            AiEntityType.GATEWAY_POLICY, cid, AiGatewayPolicy, "policy_code"
        )
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived gateway policy not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="ai_gateway_policy",
            entity_id=row.id,
