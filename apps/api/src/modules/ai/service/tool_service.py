"""ToolService — Phase 3 CRUD + publish / retire."""
...
from modules.ai.domain.enums import AiEntityType, ToolStatus
...
from modules.ai.service.engines import ToolEngine
...
class ToolService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> AiTool:
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        code = fields.pop("tool_code", None) or self._numbers.generate(
            AiEntityType.TOOL, cid, AiTool, "tool_code"
        )
        fields.setdefault("status", ToolStatus.DRAFT.value)
        row = self._repo.create(ctx, company_id=cid, tool_code=code, **fields)
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived tool not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="ai_tool",
            entity_id=row.id,
