"""GuardrailPolicyService — Phase 1 CRUD + publish / retire."""
...
from modules.ai.service.engines import GuardrailPolicyEngine
...
class GuardrailPolicyService:
    def __init__(self, db: Session) -> None:
        self._repo = GuardrailPolicyRepository(db)
        self._scope = AiScopeValidator(db)
        self._numbers = AiNumberService(db)
        self._engine = GuardrailPolicyEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> AiGuardrailPolicy:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("Guardrail policy not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        code = fields.pop("policy_code", None) or self._numbers.generate(
            AiEntityType.GUARDRAIL_POLICY, cid, AiGuardrailPolicy, "policy_code"
        )
        ...
        self._audit.log_entity_change(
            ...
            entity_name="ai_guardrail_policy",
            entity_id=row.id,
            operation="create",
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived guardrail policy not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="ai_guardrail_policy",
            entity_id=row.id,
