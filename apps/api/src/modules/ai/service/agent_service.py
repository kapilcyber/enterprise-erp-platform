"""AgentService — Phase 3 catalog CRUD + activate / deactivate."""
...
from modules.ai.domain.enums import AgentStatus, AiEntityType
...
from modules.ai.service.engines import AgentEngine
...
class AgentService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> AiAgent:
    ...
    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        code = fields.pop("agent_code", None) or self._numbers.generate(
            AiEntityType.AGENT, cid, AiAgent, "agent_code"
        )
        fields.setdefault("status", AgentStatus.ACTIVE.value)
        row = self._repo.create(ctx, company_id=cid, agent_code=code, **fields)
    ...
    def restore(self, ctx: TenantContext, row_id: UUID):
        ...
        if row is None:
            raise NotFoundException("Archived agent not found")
        self._audit.log_entity_change(
            tenant_id=ctx.tenant_id,
            entity_name="ai_agent",
            entity_id=row.id,
