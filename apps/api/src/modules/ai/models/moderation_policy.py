"""ai_moderation_policy ORM per ERD-27 Phase 1."""
...
class AiModerationPolicy(Base, *AiRowMixin):
    __tablename__ = "ai_moderation_policy"
    __table_args__ = (
        UniqueConstraint("company_id", "policy_code", name="uk_ai_moderation_policy_code"),
        CheckConstraint(
            f"status IN ({_STATUSES})",
            name="ck_ai_moderation_policy_status",
        ),
        Index("ix_ai_moderation_policy_tenant_co_status", "tenant_id", "company_id", "status"),
        Index("ix_ai_moderation_policy_name_search", "company_id", "policy_name"),
        Index("ix_ai_moderation_policy_code_search", "company_id", "policy_code"),
        {"schema": "ai"},
    )
