"""Shared AI router utilities — thin handler helpers."""
...
from modules.ai.dependencies import (
    PaginationParams,
    SortParams,
...
def register_standard_crud(
    ...
    *,
    ...
    read_perm = f"ai.{resource}:read"
    create_perm = f"ai.{resource}:create"
    update_perm = f"ai.{resource}:update"
    archive_perm = f"ai.{resource}:archive"
    restore_perm = f"ai.{resource}:restore"
