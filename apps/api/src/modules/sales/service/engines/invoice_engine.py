...
from modules.sales.domain.enums import InvoiceStatus
...
class InvoiceEngine:
    def validate_editable(self, invoice: SalesInvoiceHeader) -> None:
        if invoice.status != InvoiceStatus.DRAFT.value:
            raise InvalidDocumentState("Only draft invoices can be edited")

    def validate_postable(self, invoice: SalesInvoiceHeader) -> None:
    ...
    def compute_line_totals(self, line: SalesInvoiceLine) -> LineTotals:
    ...
    def apply_line_totals(self, line: SalesInvoiceLine, totals: LineTotals) -> None:
        line.tax_amount = float(totals.tax_amount)
        line.line_total = float(totals.line_total)

    def compute_header_totals(self, lines: list[SalesInvoiceLine]) -> DocumentTotals:
    ...
    def apply_header_totals(
        self, invoice: SalesInvoiceHeader, totals: DocumentTotals
    ) -> None:
    ...
    def refresh_balance_due(self, invoice: SalesInvoiceHeader) -> None:
