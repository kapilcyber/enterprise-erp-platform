"""Sales repository base utilities."""
...
class SalesScopedRepository(OrgScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    @staticmethod
    def apply_sales_filter(stmt, model, ctx: TenantContext, *, branch_scoped: bool = False):
        stmt = SalesScopedRepository.apply_tenant_filter(stmt, model, ctx)
    ...
    def resolve_company_id(ctx: TenantContext, company_id: UUID | None) -> UUID:
        if company_id is not None:
            SalesScopedRepository.ensure_company_access(ctx, company_id)
            return company_id
