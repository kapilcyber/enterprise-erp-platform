...
erDiagram
    ...
    vp_delivery_schedule ||--o{ vp_payment_status : fulfilled_payment
    vp_invoice_submission ||--o{ vp_payment_status : paid_as
    vp_dashboard ||--o{ vp_payment_status : summarizes
    vp_report ||--o{ vp_payment_status : summarizes
...
master_vendor / master_employee / master_product (C-01)
    └── vp_supplier_profile ── master_vendor
            └── vp_portal_account
                    ...
                    │      │      └── vp_payment_status (fulfilled_payment)
                    ├── vp_document_access ── document_id (UUID — Document SoR)
                    └── vp_report ── vp_payment_status (summary)
...
| 464 | `0464_seed_vp_workflows` | Workflows |

**Planned head:** `0464_seed_vp_workflows`
...
| 8 | Migration order `0443`–`0464`, revision IDs ≤ 32 chars; planned head `0464_seed_vp_workflows` | ✅ |
...
| 1.1 | 2026-07-20 | Editorial lock for Sprint 24 implementation planning. Mermaid/ASCII payment-status links; planned head `0464_seed_vp_workflows` (≤32 chars). |
